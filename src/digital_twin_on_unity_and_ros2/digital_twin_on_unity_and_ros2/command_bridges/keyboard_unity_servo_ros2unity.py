#!/usr/bin/env python3
"""Unity-driven keyboard teleoperation servo (port of keyboard_servo)."""

# This is a port of teleoperation_general's keyboard_servo whose mapping,
# home/controller-switch/servo machinery is copied verbatim. Only the input
# layer changes: instead of reading raw terminal stdin (which Unity cannot feed
# into a WSL terminal), it subscribes to /unity/teleop/keys
# (digital_twin_interfaces/TeleopKeysUnity) and publishes a CARTESIAN_TWIST at a
# fixed rate from the set of currently-held jog keys (so motion is continuous
# while a key is held and zeroes on release without flicker).
#
# Sync discipline: this node is a copy of keyboard_servo's mapping and advanced
# features. If the upstream changes key semantics or controller names, re-copy
# the affected logic here and in keyboard_jog.py (same rule as the
# third_party/teleop_msgs whole-package re-vendor discipline).
#
# Why a manual loop instead of rclpy timers: keyboard_servo's
# home/controller/servo methods call spin_until_future_complete synchronously.
# Running them from an executor callback would re-enter the executor; instead
# main() pumps spin_once and then runs queued actions and the periodic publish
# from the loop body, exactly mirroring keyboard_servo's original architecture.
...
class ControllerMode(Enum):
    """Active motion controller mode reported by controller_manager."""
...
class KeyboardUnityServoROS2Unity(Node):
    """Convert Unity keyboard input into TeleopCommand messages."""

    def __init__(self) -> None:
        super().__init__("keyboard_unity_servo")

        self.declare_parameter("keys_topic", "/unity/teleop/keys")
        ...
        self.keys_topic = self.get_parameter("keys_topic").value
        ...
        self.home_joint_positions = self._double_array_parameter(
            "home_joint_positions"
        )
        self.home_duration_s = float(
            self.get_parameter("home_duration_s").value
        )
        self.home_trajectory_action = self.get_parameter(
            "home_trajectory_action"
        ).value
        ...
        self.auto_switch_for_home = bool(
            self.get_parameter("auto_switch_for_home").value
        )
        self.return_to_servo_after_home = bool(
            self.get_parameter("return_to_servo_after_home").value
        )
        self.joint_state_topic = self.get_parameter("joint_state_topic").value
        self.servo_command_topic = self.get_parameter(
            "servo_command_topic"
        ).value
        ...
        self.servo_node_prefix = self.get_parameter(
            "servo_node_prefix"
        ).value.rstrip("/")
        self.last_joint_state: JointState | None = None

        # Unity input state: jog keys held now, and a queue of momentary
        # action keys to run from the main loop (never inside a callback).
        self.held_keys = ""
        ...
        self.get_logger().info(
            f"reading Unity keyboard input from {self.keys_topic}"
        )
        self.get_logger().info(
            f"publishing TeleopCommand on {self.command_topic} "
            f"at {self.publish_rate:.1f} Hz"
        )
        self.get_logger().info(
            f"reading current EE pose from {self.observation_topic}"
        )
        self.get_logger().info(
    ...
    def on_keys(self, msg: TeleopKeysUnity) -> None:
        """Update held jog keys and queue any momentary action for the loop."""
        self.held_keys = filter_held_keys(msg.held_keys)
        if msg.action_key:
            self._pending_actions.append(msg.action_key)

    def publish_tick(self) -> None:
        """Publish one fixed-rate twist from the currently-held jog keys."""
        linear, angular = held_keys_to_twist(
            self.held_keys, self.linear_step, self.angular_step
        )
        self.publish_command(linear=linear, angular=angular, log=False)

    def process_pending_actions(self) -> None:
        """Run queued momentary actions from the main loop (not a callback)."""
    ...
    def _dispatch_action(self, action: str) -> None:
        ...
        elif action == ACTION_TOGGLE_CONTROLLER:
            self.toggle_motion_controller()
        elif action == ACTION_RECORD_HOME:
            self.record_current_as_home()
        elif action == ACTION_RECORD_ZERO:
            self.record_current_as_zero()
        elif action == ACTION_STOP_AND_CLEAR:
            self.held_keys = ""
            self.publish_command(mode=TeleopCommand.IDLE)
        else:
            self.get_logger().warning("unknown action key: %r" % action)
    ...
    def publish_command(
        ...
        *,
        ...
        """Build and publish a TeleopCommand (twist by default)."""
    ...
    def on_observation(self, msg: TeleopObservation) -> None:
        """Cache the latest end-effector pose for record/home actions."""
    ...
    def on_joint_state(self, msg: JointState) -> None:
        """Cache the latest joint state for servo seeding."""
        self.last_joint_state = msg

    def publish_home_pose(self) -> None:
        """Drive to the configured home (joint trajectory or cartesian pose)."""
        ...
        if self.home_pose is None:
            self.get_logger().warning(
                "home pose is not set; configure home_position or press r "
                "after a current pose is available"
            )
            return

        self.publish_command(
            mode=TeleopCommand.CARTESIAN_POSE, pose=self.home_pose
        )

    def send_home_joint_trajectory(self) -> None:
        """Send a FollowJointTrajectory goal to the configured home joints."""
        if len(self.home_joint_names) != len(self.home_joint_positions):
            self.get_logger().error(
                "home_joint_names and home_joint_positions must have the "
                "same length"
            )
            return
        ...
        if self.auto_switch_for_home:
            ...
            restore_servo = (
                mode == ControllerMode.SERVO
                and self.return_to_servo_after_home
            )
        ...
        if not self.home_action_client.wait_for_server(timeout_sec=0.2):
            self.get_logger().error(
                "home action server is not available: "
                f"{self.home_trajectory_action}"
            )
            return
    ...
    def toggle_motion_controller(self) -> None:
        """Toggle between the trajectory and servo motion controllers."""
        mode = self.get_controller_mode()
        if mode == ControllerMode.TRAJECTORY:
            self.switch_to_servo_controller()
        else:
    ...
    def stop_moveit_servo(self) -> bool:
        """Stop the MoveIt servo node if servo management is enabled."""
    ...
    def start_moveit_servo(self) -> bool:
        """Start the MoveIt servo node if servo management is enabled."""
        if not self.manage_servo_node_on_switch:
            return True
        return self.call_trigger_service(
            self.start_servo_client, "start servo"
        )

    def reset_moveit_servo_status(self) -> bool:
        """Reset the MoveIt servo status if servo management is enabled."""
        if not self.manage_servo_node_on_switch:
            return True
        if not self.reset_servo_status_client.wait_for_service(
            timeout_sec=0.2
        ):
            self.get_logger().warning(
                "reset servo status service is not available"
            )
            return False
    ...
    def call_trigger_service(self, client, label: str) -> bool:
        """Call a std_srvs/Trigger service and report success/failure."""
        ...
        if not response.success:
            self.get_logger().warning(
                "%s returned false: %s" % (label, response.message)
            )
            return False
    ...
    def current_joint_positions_for_servo(self) -> list[float] | None:
        """Read the joints needed to seed the servo controller, or None."""
        for _ in range(5):
    ...
    def publish_servo_seed(
        ...
        *,
        ...
        """Publish current joints to the servo command topic as a seed."""
        publish_count = (
            count if count is not None else self.servo_seed_publish_count
        )
        ...
        self.get_logger().info(
            "seeded servo target with current joints: %s" % positions
        )

    def get_controller_mode(self) -> ControllerMode:
        """Query controller_manager for the active motion controller mode."""
        ...
        trajectory_active = (
            states.get(self.trajectory_controller) == "active"
        )
        servo_active = states.get(self.servo_controller) == "active"

        if trajectory_active:
            self.get_logger().info(
                f"active controller: {self.trajectory_controller}"
            )
            return ControllerMode.TRAJECTORY
        if servo_active:
            self.get_logger().info(
                f"active controller: {self.servo_controller}"
            )
            return ControllerMode.SERVO

        self.get_logger().warning(
            "neither configured motion controller is active: %s" % states
        )
        return ControllerMode.UNKNOWN
    ...
    def switch_controllers(
        ...
        *,
        ...
        """Switch controller_manager controllers, returning success."""
    ...
    def record_current_as_home(self) -> None:
        """Record the current end-effector pose as the home pose."""
    ...
    def record_current_as_zero(self) -> None:
        """Record the current end-effector pose as the zero/neutral pose."""
        current = self._fresh_current_pose()
        if current is None:
            return
        self.zero_pose = deepcopy(current)
        self.get_logger().info(
            "recorded current end-effector pose as zero/neutral"
        )
    ...
    def _make_home_pose(self) -> PoseStamped | None:
        ...
        if len(position) != 3:
            self.get_logger().warning(
                "home_position must contain exactly 3 values"
            )
            return None
    ...
    def _home_orientation_xyzw(
        ...
        if rpy:
            if len(rpy) != 3:
                self.get_logger().warning(
                    "home_rpy must contain exactly 3 values"
                )
                return None
            return self._quaternion_from_rpy(
    ...
    def spin_unity_keyboard(self) -> None:
        """Pump callbacks, run queued actions, and publish at a fixed rate."""
        period = 1.0 / self.publish_rate
        next_publish = self.get_clock().now()
        while rclpy.ok():
            ...
            if now >= next_publish:
                self.publish_tick()
                next_publish = now + Duration(seconds=period)


def main(args: list[str] | None = None) -> None:
    """Run the Unity keyboard servo node with its manual publish loop."""
    rclpy.init(args=args)
    node = KeyboardUnityServoROS2Unity()

    try:
        node.spin_unity_keyboard()
