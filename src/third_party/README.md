...
| `ROS-TCP-Endpoint` | Unity-Technologies/ROS-TCP-Endpoint (git clone) | local clone | **no (gitignored)** | large clone with its own upstream; managed as a clone, not committed |

## Tracking policy

For offline / clone-and-build reproducibility, the three small msg packages
(`ur_dashboard_msgs`, `ur_msgs`, `teleop_msgs`) are committed into the repo.
`ROS-TCP-Endpoint` stays gitignored (`.gitignore`:
`src/third_party/ROS-TCP-Endpoint/`) and is restored by cloning.
